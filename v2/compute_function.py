...
def calculate_compute_cost(start_time, end_time, instance_type, region_name, instance_description='Linux/UNIX', availability_zone='us-west-2b'):
    ...
    if(not instance_type):
        ...
        try:
            ...
            endDatetime = dt.datetime.strptime(end_time, "%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=p.UTC) + dt.timedelta(hours=2)
        ...
        response = client.describe_spot_price_history(AvailabilityZone=availability_zone, MaxResults=999, InstanceTypes=[str(instance_type)], StartTime=startDatetime, EndTime=endDatetime, ProductDescriptions=[str(instance_description)])
        ...
        for spot_price in reversed(response['SpotPriceHistory']):
            ...
            if(spot_price['Timestamp'] < startDatetime):
                ...
                if((startDatetime <= spot_price['Timestamp']) and (startDatetime >= last_time_checked['Timestamp'])):
                    ...
                    price += Decimal(rate)
                    last_time_checked = spot_price
                elif((startDatetime < last_time_checked['Timestamp']) and (spot_price['Timestamp'] > last_time_checked['Timestamp']) ):
